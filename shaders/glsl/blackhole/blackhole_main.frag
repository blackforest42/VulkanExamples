...
uniform float time;
uniform samplerCube galaxy_skybox;
...
const float adiskEnabled = 1.0;
const float adiskParticle = 1.0;
const float adiskHeight = 0.55;
const float adiskLit = 0.25;
const float adiskDensityV = 2.0;
const float adiskDensityH = 4.0;
const float adiskNoiseScale = .8;
const float adiskNoiseLOD = 5.0;
const float adiskSpeed = 0.5;
...
vec3 traceColor(vec3 pos, vec3 dir) {
  ...
  color += texture(galaxy_skybox, dir).rgb * alpha;
  return color;
}
