/*
 * Vulkan Example - Rendering a glTF model using hardware accelerated ray
 * tracing example (for proper transparency, this sample does frame
 * accumulation)
 *
  ...
  */
  void createBottomLevelAccelerationStructure() {
    ...
    VK_CHECK_RESULT(vulkanDevice_->createBuffer(
        ...
        &transformBuffer,
        static_cast<uint32_t>(transformMatrices.size()) *
        ...
        for (auto primitive : node->mesh->primitives) {
          if (primitive->indexCount > 0) {
            ...
            indexBufferDeviceAddress.deviceAddress =
                getBufferDeviceAddress(model.indices.buffer) +
            ...
            transformBufferDeviceAddress.deviceAddress =
                ...
                static_cast<uint32_t>(geometryNodes.size()) *
                    sizeof(VkTransformMatrixKHR);
