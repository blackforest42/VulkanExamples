...
class VulkanExample : public VulkanExampleBase {
  ...
  */
  RayTracingScratchBuffer createScratchBuffer(VkDeviceSize size) {
    ...
    vkGetBufferMemoryRequirements(device_, scratchBuffer.handle,
                                  &memoryRequirements);
    ...
    VK_CHECK_RESULT(vkBindBufferMemory(device_, scratchBuffer.handle,
    ...
    scratchBuffer.deviceAddress =
        vkGetBufferDeviceAddressKHR(device_, &bufferDeviceAddressInfo);
  ...
  /*
          Gets the device address from a buffer that's required for some of the
     buffers used for ray tracing
  */
  ...
  /*
          Set up a storage image that the ray generation shader will be writing
     to
  */
  void createStorageImage() {
    ...
    VK_CHECK_RESULT(
        vkCreateImage(device_, &image, nullptr, &storageImage.image));
    ...
    VK_CHECK_RESULT(vkAllocateMemory(device_, &memoryAllocateInfo, nullptr,
                                     &storageImage.memory));
    VK_CHECK_RESULT(
        vkBindImageMemory(device_, storageImage.image, storageImage.memory, 0));
  ...
  */
  void createBottomLevelAccelerationStructure() {
    ...
    VK_CHECK_RESULT(vulkanDevice_->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &transformBuffer, sizeof(VkTransformMatrixKHR), &transformMatrix));
    ...
    vertexBufferDeviceAddress.deviceAddress =
        getBufferDeviceAddress(vertexBuffer.buffer);
    indexBufferDeviceAddress.deviceAddress =
        getBufferDeviceAddress(indexBuffer.buffer);
    transformBufferDeviceAddress.deviceAddress =
        getBufferDeviceAddress(transformBuffer.buffer);
  ...
  */
  void createTopLevelAccelerationStructure() {
    ...
    */
    ...
    topLevelAS.deviceAddress = vkGetAccelerationStructureDeviceAddressKHR(
        device_, &accelerationDeviceAddressInfo);
  ...
  */
  void createShaderBindingTable() {
    ...
    memcpy(raygenShaderBindingTable.mapped, shaderHandleStorage.data(),
           handleSize);
  ...
  */
  void createDescriptorSets() {
    ...
    // Acceleration structure and storage image does not need to be duplicated
    // per frame, we use the same for each descriptor to keep things simple
    ...
    for (auto i = 0; i < MAX_CONCURRENT_FRAMES; i++) {
      ...
      std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
          accelerationStructureWrite, resultImageWrite, uniformBufferWrite};
    ...
    */
    ...
    // Ray generation group
    {
      shaderStages.push_back(
          loadShader(getShadersPath() + "raytracingbasic/raygen.rgen.spv",
                     VK_SHADER_STAGE_RAYGEN_BIT_KHR));
    ...
    {
      shaderStages.push_back(
          loadShader(getShadersPath() + "raytracingbasic/miss.rmiss.spv",
    ...
    {
      shaderStages.push_back(
          loadShader(getShadersPath() + "raytracingbasic/closesthit.rchit.spv",
  ...
  */
  ...
  void prepare() {
    ...
    // Get ray tracing pipeline properties, which will be used later on in the
    // sample
    ...
    vkBuildAccelerationStructuresKHR =
        reinterpret_cast<PFN_vkBuildAccelerationStructuresKHR>(
            vkGetDeviceProcAddr(device_, "vkBuildAccelerationStructuresKHR"));
  ...
  void buildCommandBuffer() {
    ...
    */
    ...
    raygenShaderSbtEntry.deviceAddress =
        getBufferDeviceAddress(raygenShaderBindingTable.buffer);
    raygenShaderSbtEntry.stride = handleSizeAligned;
    raygenShaderSbtEntry.size = handleSizeAligned;

    VkStridedDeviceAddressRegionKHR missShaderSbtEntry{};
    missShaderSbtEntry.deviceAddress =
        getBufferDeviceAddress(missShaderBindingTable.buffer);
    missShaderSbtEntry.stride = handleSizeAligned;
    missShaderSbtEntry.size = handleSizeAligned;

    VkStridedDeviceAddressRegionKHR hitShaderSbtEntry{};
    hitShaderSbtEntry.deviceAddress =
        getBufferDeviceAddress(hitShaderBindingTable.buffer);
    ...
    */
    ...
    vkCmdTraceRaysKHR(cmdBuffer, &raygenShaderSbtEntry, &missShaderSbtEntry,
                      &hitShaderSbtEntry, &callableShaderSbtEntry, width_,
                      height_, 1);
