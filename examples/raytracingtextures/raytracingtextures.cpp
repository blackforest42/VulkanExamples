 ...
 * to a closest hit shader, that now samples from the texture, an any hit shader
 * is added to the closest hit shader group. We use this shader to check if the
 * texel we want to sample at the currently hit ray position is transparent, and
 * if that's the case the any hit shader will cancel the intersection.
 */
