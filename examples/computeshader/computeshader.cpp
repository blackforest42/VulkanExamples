...
public:
	...
	struct Graphics {
		...
		std::array<DescriptorSets, MAX_CONCURRENT_FRAMES> descriptorSets;
	...
	struct Compute {
		...
		std::array<VkCommandBuffer, MAX_CONCURRENT_FRAMES> commandBuffers;// Command buffers storing the dispatch commands and barriers
	...
	void prepareStorageImage()
	{
		...
		VK_CHECK_RESULT(vkCreateImage(device_, &imageCreateInfo, nullptr, &storageImage.image));
		...
		VK_CHECK_RESULT(vkAllocateMemory(device_, &memAllocInfo, nullptr, &storageImage.deviceMemory));
		VK_CHECK_RESULT(vkBindImageMemory(device_, storageImage.image, storageImage.deviceMemory, 0));
		...
		VK_CHECK_RESULT(vkCreateSampler(device_, &sampler, nullptr, &storageImage.sampler));
		...
		VK_CHECK_RESULT(vkCreateImageView(device_, &view, nullptr, &storageImage.view));
	...
	void loadAssets()
	{
		textureColorMap.loadFromFile(getAssetPath() + "textures/vulkan_11_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice_, queue_, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
	}
	...
	void prepareGraphics()
	{
		...
		for (auto i = 0; i < graphics.uniformBuffers.size(); i++) {
			...
			VK_CHECK_RESULT(vkAllocateDescriptorSets(device_, &allocInfo, &graphics.descriptorSets[i].preCompute));
			...
			vkUpdateDescriptorSets(device_, static_cast<uint32_t>(baseImageWriteDescriptorSets.size()), baseImageWriteDescriptorSets.data(), 0, nullptr);

			// Final image (after compute shader processing)
			VK_CHECK_RESULT(vkAllocateDescriptorSets(device_, &allocInfo, &graphics.descriptorSets[i].postCompute));
	...
	void prepareCompute()
	{
		...
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device_, &allocInfo, &compute.descriptorSet));
		...
		for (auto& shaderName : filterNames) {
			...
			VkPipeline pipeline;
			VK_CHECK_RESULT(vkCreateComputePipelines(device_, pipelineCache_, 1, &computePipelineCreateInfo, nullptr, &pipeline));
	...
	void prepareUniformBuffers()
	{
		for (auto& buffer : graphics.uniformBuffers) {
			...
			VK_CHECK_RESULT(vulkanDevice_->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &buffer, sizeof(Graphics::UniformData)));
	...
	void buildGraphicsCommandBuffer()
	{
		...
		VkViewport viewport = vks::initializers::viewport((float)width_ * 0.5f, (float)height_, 0.0f, 1.0f);
		...
		// Left (pre compute)
		vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphics.pipelineLayout, 0, 1, &graphics.descriptorSets[currentBuffer_].preCompute, 0, nullptr);
		vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphics.pipeline);

		vkCmdDrawIndexed(cmdBuffer, indexCount, 1, 0, 0, 0);

		// Right (post compute)
		vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphics.pipelineLayout, 0, 1, &graphics.descriptorSets[currentBuffer_].postCompute, 0, nullptr);
		vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphics.pipeline);

		viewport.x = (float)width_ / 2.0f;
		vkCmdSetViewport(cmdBuffer, 0, 1, &viewport);
		vkCmdDrawIndexed(cmdBuffer, indexCount, 1, 0, 0, 0);
